...
module mt48lc16m16a2 (Dq, Addr, Ba, Clk, Cke, Cs_n, Ras_n, Cas_n, We_n, Dqm, debug);
...
`endif
   ...
   reg [31 : 0] 		 Bank0_32bit [0 : (mem_sizes/2)*4]; // Temporary 32-bit wide array to hold readmemh()'d data before loading into 16-bit wide array
    ...
    wire      Debug            = debug;                          // Debug messages : 1 = On
    ...
    parameter tHZ  =   5.0;
    parameter tOH  =   2.5;
    parameter tMRD =   12.0;     // 2 Clk Cycles
    parameter tRAS =  42.0;
    parameter tRC  =  60.0;
    parameter tRCD =  18.0;
    parameter tRFC =  60.0;
...
`ifdef PRELOAD_RAM // Added jb
       ...
       $readmemh("sram.vmem", Bank0_32bit);
		...
		for (mem_cnt=0;mem_cnt < (mem_sizes/2); mem_cnt = mem_cnt + 1) begin
			Bank0[(mem_cnt_x*2)] = Bank0_32bit[mem_cnt][15:0];
			Bank0[(mem_cnt_x*2)+1] = Bank0_32bit[mem_cnt][31:16];
			mem_cnt_x = mem_cnt_x + 1;
		end
		 mem_cnt_x=0;
		for (mem_cnt=(mem_sizes/2);mem_cnt < (mem_sizes/2)*2; mem_cnt = mem_cnt + 1) begin
			Bank1[(mem_cnt_x*2)] = Bank0_32bit[mem_cnt][15:0];
			Bank1[(mem_cnt_x*2)+1] = Bank0_32bit[mem_cnt][31:16];
			mem_cnt_x = mem_cnt_x + 1;
		end
		 mem_cnt_x=0;
		for (mem_cnt=(mem_sizes/2)*2;mem_cnt < (mem_sizes/2)*3; mem_cnt = mem_cnt + 1) begin
			Bank2[(mem_cnt_x*2)] = Bank0_32bit[mem_cnt][15:0];
			Bank2[(mem_cnt_x*2)+1] = Bank0_32bit[mem_cnt][31:16];
			mem_cnt_x = mem_cnt_x + 1;
		end
		 mem_cnt_x=0;
		for (mem_cnt=(mem_sizes/2)*3;mem_cnt < (mem_sizes/2)*4; mem_cnt = mem_cnt + 1) begin
			Bank3[(mem_cnt_x*2)] = Bank0_32bit[mem_cnt][15:0];
			Bank3[(mem_cnt_x*2)+1] = Bank0_32bit[mem_cnt][31:16];
			mem_cnt_x = mem_cnt_x + 1;
		end
