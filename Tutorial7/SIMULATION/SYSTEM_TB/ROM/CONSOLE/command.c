...
void cmd_enter(void) {
	...
	if (strncmp(cmd, "HELP", 4) == 0) {
		...
		return;
	}else if (strncmp(cmd, "BLACKBUF", 8) == 0) {
		black_video_mode_0_mem();
		outputstring("BUFFER BLACKED", true);
	}else if (strncmp(cmd, "WHITEBUF", 8) == 0) {
		white_video_mode_0_mem();
		outputstring("BUFFER WHITED", true);
	}else if (strncmp(cmd, "SWAPOFF", 7) == 0) {
		set_video_auto_swap_off();
		outputstring("AUTO SWAP OFF", true);
	}else if (strncmp(cmd, "SWAPON", 6) == 0) {
		set_video_auto_swap_on();
		outputstring("AUTO SWAP OFF", true);
	}
	else if (strncmp(cmd, "SWAPNOW", 7) == 0) {
		if (current_buffer == 0) {
			swap_buffers_now();
			outputstring("SWAPPED TO B BUFFER", true);
		}else if (current_buffer == 1) {
			swap_buffers_now();
			outputstring("SWAPPED TO A BUFFER", true);
		}
